<template>
  <div class="app-container">
    ...
    <el-form ref="form" :model="form" :inline="true" label-width="120px">
      <el-form-item label="命名空间"> <!-- namespace 输入框 -->
        <el-input v-model="form.namespace" />
      </el-form-item>
      <el-form-item label="PodName"> <!-- pod 名称输入框 -->
        <el-input v-model="form.pod_name" />
      </el-form-item>
      <el-form-item label="ContainerName"> <!-- pod 名称输入框 -->
...
export default {
  data() {
    return {
      form: {
        ...
        pod_name: 'example-deployment-6b86d96995-4gz52', // 默认 shell 命令为 "bash"
        container_name: 'example-container', // 默认容器名称为 "nginx"
  ...
  methods: {
    onSubmit() {
      ...
      const ws = new WebSocket('ws://127.0.0.1:8888/api/k8s/sshpod?namespace=' + this.form.namespace + '&pod_name=' + this.form.pod_name + '&container_name=' + this.form.container_name + '&command=' + this.form.command)
